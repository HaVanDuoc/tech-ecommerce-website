...
import { Box, Button, Dialog, Divider, Grid, Rating, Slide, Stack, styled, Typography } from "@mui/material"
        ...
        const fetch = async () => {
            ...
            setPayment(
                Number(response.data.data.price) -
                    Number(response.data.data.price) *
                        ((Number(response.data.data.discount) ? Number(response.data.data.discount) : 0) / 100)
            )
        }
                        ...
                        >
                            {fetch?.data &&
                                JSON.parse(fetch?.data?.image).map((item, index) => {
                                    return <img src={item?.base64} alt={item?.fileName} key={index} />
                                })}
                        </Slider>
                    </Box>
                </Box>
                ...
                <Grid item xs={6}>
                    <Box className="right">
                        ...
                        >
                            ...
                            <Rating name="read-only" value={4} readOnly sx={{ marginLeft: 1, marginRight: 1 }} />
...
const BoxCount = ({ payment, setPayment, price, discount }) => {
    const handleDecrease = () => {
        ...
        const currentValue = document.querySelector(`.box-quantity .count`).innerHTML

        // if value = 1 then stop
        if (currentValue === "1") return

        // Increase it by 1 then render
        document.querySelector(`.box-quantity .count`).innerHTML = Number(currentValue) - 1
