  ...
  try {
    ...
    let [orderList] = await db.sequelize.query(`
            ...
            offset
                ${offset};
        `);
    await Promise.all(orderList.map(async item => {
...
exports.destroyOrder = async data => {
  try {
    ...
    if (response[0].status === "Đã hủy") {
      await db.order_details.update({
        status_id: 1
      }, {
        where: {
          id: data.order_details_id
        }
      });
      return {
        err: 0,
...
exports.handleOrderStatus = (actionConfirm, actionConfirmed, codeOrder) => new Promise(async (resolve, reject) => {
  try {
    let status_id;
    ...
    switch (actionConfirmed) {
      ...
      case actionConfirm.TRA_HANG:
        ...
        break;
      case actionConfirm.MUA_LAI:
        // Trường hợp đã trả hàng sau khi "xác nhận mua lại" chuyển sang "chờ lấy hàng" ko cần "chờ xác nhận" nữa
        status_id = 2; // Chờ xác nhận
        break;
      default:
        return;
    ...
    const updateStatus = await db.order_details.update({
      status_id
    }, {
      where: {
        code: codeOrder
    ...
    if (actionConfirmed === actionConfirm.DA_GIAO) {
      const [transactionVolume] = await db.sequelize.query(`select transactionVolume from users where id = ${user_id}`);
      const newTransactionVolume = Number(transactionVolume[0].transactionVolume) + Number(order_totalPayment);
      await db.sequelize.query(`update users set transactionVolume = ${newTransactionVolume} where id = ${user_id}`);
    }
    resolve({
...
exports.handleIncrease = async req => {
  try {
    ...
    const updateCount = await db.sequelize.query(`update order_items set quantity = quantity + 1, pay = pay + ${pay} where id = ${order_items_id}`);
    const updateSumOrder = await db.sequelize.query(`update order_details set total = total + ${pay} where id = ${order_detail_id}`);
    return {
...
exports.handleAddProduct = async (order_detail_id, product_id, quantity) => {
  try {
    ...
    if (created) {
      const update = await db.sequelize.query(`update order_details set total = total + ${priceOfProduct} where id = ${order_detail_id}`);
    }
    return {
      err: created ? 0 : 1,
...
exports.handleDelete = async req => {
  try {
    ...
    const updateTotalPaymentOfOrder = await db.sequelize.query(`update order_details set total = total - ${payOfProduct.pay} where id = ${order_detail_id}`);

    // Finally, delete product in table order_items
    const deleteItem = await db.order_items.destroy({
      where: {
        id: order_items_id
      }
    });
