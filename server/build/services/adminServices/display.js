...
exports.createNewCategory = data => new Promise(async (resolve, reject) => {
  try {
    const {
      name,
      image,
    ...
    resolve({
      err: response[1] ? 0 : 1,
      msg: response[1] ? "Đã thêm danh mục mới!" : `Danh mục ${name} đã tồn tại!`,
...
exports.getCategory = categoryId => new Promise(async (resolve, reject) => {
  try {
    ...
    if (Array.isArray(response) && !response.length) return resolve({
      err: 1,
      msg: "Not find object",
      data: response[0]
    });
...
exports.updateCategory = (categoryId, data) => new Promise(async (resolve, reject) => {
  try {
    const {
      brands,
      name
    ...
    if (Array.isArray(brands) && brands.length > 0) {
      ...
      const [responseIdCategory] = await sequelize.query(`select id from categories where categoryId = "${categoryId}"`);
...
exports.createNewBrand = data => new Promise(async (resolve, reject) => {
  try {
    const {
      name,
      logo,
    ...
    resolve({
      err: response[1] ? 0 : 1,
      msg: response[1] ? "Đã thêm thương hiệu mới!" : `Thương hiệu ${name} đã tồn tại!`,
      data: response[1] ? response[0] : null
    });
  } catch (error) {
    reject(error);
...
exports.setBrandForCategories = (categoryId, data) => new Promise(async (resolve, reject) => {
  try {
    const [responseIdCategory] = await sequelize.query(`select id from categories where categoryId = "${categoryId}"`);
    const idCategory = responseIdCategory[0].id; //

    const [responseIdBrand] = await sequelize.query(`select id from brands where name = "${data.name}"`);
    const idBrand = responseIdBrand[0].id; //
    ...
    resolve({
      err: response[1] ? 0 : 1,
      msg: response[1] ? "Thành công!" : "Thương hiệu này đã có trong danh mục!",
      data: response[1] ? response[0] : null
    });
  } catch (error) {
    reject(error);
...
exports.selectedBrands = categoryId => new Promise(async (resolve, reject) => {
  try {
    const [responseIdCategory] = await sequelize.query(`select id from categories where categoryId = "${categoryId}"`);
    const idCategory = responseIdCategory[0].id;
    const query = `select
