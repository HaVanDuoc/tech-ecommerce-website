...
exports.headerSaveRecent = async (product_id, user_id) => {
  try {
    ...
    const [countRecent] = await db.sequelize.query(`select count(*) as 'count' from searches where user_id = "${user_id}"`);
    if (countRecent[0].count === 20) {
      ...
      // update
      var updateRecent = await db.searches.create({
        user_id,
        product_id
      });
    } else {
