...
exports.getCurrentUser = async user_id => {
  try {
    ...
    const getSumPayment = async () => {
      const [sum] = await db.sequelize.query(`select sum(total) as sum from order_details where user_id = ${user_id}`);
      return sum[0].sum;
    };
...
exports.register = async data => {
  try {
    ...
    const [lastId] = await db.sequelize.query(query, {
      raw: true
    }); // Get uid of user final e.g 'U00000006'
    ...
    const response = await db.users.findOrCreate({
      ...
      // tìm thấy email created=false -> Tài khoản đã tồn tại
...
exports.login = async data => {
  try {
    ...
    const token = isCheckedPassword ? jwt.sign({
      id: response.id,
...
exports.getCode = async req => {
  try {
    ...
    if (updateCode) {
      mailer.sendMail(email, `${code} là mã khôi phục tài khoản Tech của bạn`, mailVerifyCode({
        code
      }));
    }
    return {
...
exports.verifyCode = async req => {
  try {
    ...
    const getCode = await db.users.findOne({
      where: {
        email
      },
      attributes: ["code_verify"],
      raw: true
    });
    ...
    if (decoded.name === "TokenExpiredError") return {
      err: 1,
      msg: "Mã xác nhận đã quá hạn!"
    };
    if (Number(code) !== Number(decoded.code)) return {
      err: 1,
